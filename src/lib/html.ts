...
export async function renderHtmlEmail(
  ...
  const messageMarkdownFragment = renderTemplate(
  ...
  if (messageContainer) {
    messageContainer.innerHTML = await markedInstance.parse(
      messageMarkdownFragment
    );
  } else {
